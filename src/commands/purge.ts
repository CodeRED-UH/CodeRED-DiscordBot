...
export const purge: Command = {
    ...
    run: async (interaction, client) => {
        ...
        const guildMember = await interaction.guild.members.cache.find(u => u.id === user.id);

        if (!guildMember) {
            await interaction.editReply("Failed to retrieve your information in this guild.");
            return;
        }
        ...
        await channel?.bulkDelete(n, true)
            .catch((err: Error) => {
